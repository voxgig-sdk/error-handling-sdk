...
//     (\`new ErrorHandlingSDK(...)\` / \`ErrorHandlingSDK.test(...)\` become
//     \`ErrorHandlingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ErrorHandlingSDK'
...
// rewrite real client construction (\`new ErrorHandlingSDK(...)\` and any
// \`ErrorHandlingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ErrorHandlingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ErrorHandlingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
