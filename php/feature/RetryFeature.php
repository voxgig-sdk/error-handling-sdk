...
// ErrorHandling SDK retry feature
...
class ErrorHandlingRetryFeature extends ErrorHandlingBaseFeature
{
    ...
    public function init(ErrorHandlingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ErrorHandlingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ErrorHandlingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ErrorHandlingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
