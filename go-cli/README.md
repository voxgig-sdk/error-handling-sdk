...
boru-driven command-line client **and** interactive REPL for the ErrorHandling
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/error-handling-cli)
make build

# 2. See usage (words, entities, env vars)
./error-handling-cli --help

# 3. Provide credentials once, via the environment
export ERROR_HANDLING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./error-handling-cli load 1 logo_generation            # {id:1} shorthand
./error-handling-cli load '{id:1}' logo_generation       # explicit match map

# 5. Override the API base URL for a single call
ERROR_HANDLING_BASE=https://api.example.com ./error-handling-cli load 1 logo_generation

# 6. No arguments -> interactive REPL
./error-handling-cli
error-handling> load 1 logo_generation
error-handling> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/error-handling-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/error-handling-cli load 1 logo_generation
   ```
...
```sh
./error-handling-cli load 1 logo_generation          # scalar shorthand for {id:1}
./error-handling-cli load '{id:1}' logo_generation     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `error-handling>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/error-handling-cli`. |
...
(`./error-handling-cli <expr>`) and interactively (the REPL), and expressions compose the
