...
require_relative 'ErrorHandling_types'


class ErrorHandlingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ErrorHandlingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ErrorHandlingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ErrorHandlingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ErrorHandlingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ErrorHandlingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ErrorHandlingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ErrorHandlingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ErrorHandlingError.new(
        "#{op}_allow",
        "ErrorHandlingSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ErrorHandlingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ErrorHandlingError.new(
        "graphql_error", "ErrorHandlingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ErrorHandlingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
