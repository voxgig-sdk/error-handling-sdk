...
module ErrorHandlingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ErrorHandlingResponse.new({})
    ...
    if fetch_err
      ...
      response = ErrorHandlingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ErrorHandlingResponse.new(fetched)
    else
