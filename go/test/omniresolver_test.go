...
import (
	...
	omni "github.com/voxgig-sdk/error-handling-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ErrorHandlingError); is {
				out := map[string]any{
					"name":    "ErrorHandlingError",
...
func omniCtx(arg any, client *sdk.ErrorHandlingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ErrorHandlingSDK); is {
			client = live
