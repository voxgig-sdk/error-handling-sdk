...
[MCP](https://modelcontextprotocol.io) server exposing the ErrorHandling SDK as
two agent tools — `error-handling_list` and `error-handling_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/error-handling-mcp)
make build

# 2. Provide credentials via the environment
export ERROR_HANDLING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user error-handling \
  -- /absolute/path/to/error-handling-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./error-handling-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// error-handling_list: first page of records
{ "entity": "logo_generation" }
{ "entity": "logo_generation", "query": { } }

// error-handling_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/error-handling-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user error-handling \
     -- "$PWD"/dist/*/error-handling-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `error-handling_list` and `error-handling_load` tools now appear
   in new sessions. Ask the agent to *"list logo_generation using error-handling"*
   and it calls `error-handling_list` with `{"entity":"logo_generation"}`.
...
```sh
export ERROR_HANDLING_APIKEY=sk_live_xxx            # API key
export ERROR_HANDLING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `error-handling_list` tool
...
### Call the `error-handling_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `error-handling_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `error-handling_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ERROR_HANDLING_APIKEY` | API key sent with every request. |
| `ERROR_HANDLING_BASE` | Optional override of the API base URL. |
...
```sh
./error-handling-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"error-handling_load","arguments":{"entity":"logo_generation","query":{"id":1}}}}'
```
