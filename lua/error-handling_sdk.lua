...
local ErrorHandlingSDK = {}
ErrorHandlingSDK.__index = ErrorHandlingSDK
...
ErrorHandlingSDK._make_feature = _make_feature


function ErrorHandlingSDK.new(options)
  local self = setmetatable({}, ErrorHandlingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ErrorHandlingSDK:options_map()
...
function ErrorHandlingSDK:get_utility()
  return Utility.copy(self._utility)
end


function ErrorHandlingSDK:get_root_ctx()
  return self._rootctx
end


function ErrorHandlingSDK:prepare(fetchargs)
...
function ErrorHandlingSDK:direct(fetchargs)
...
-- by passing it.
function ErrorHandlingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ErrorHandlingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ErrorHandlingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ErrorHandlingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ErrorHandlingSDK.new(sdkopts)
  sdk.mode = "test"
