...
describe("LogoGenerationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ERROR_HANDLING_TEST_LOGO_GENERATION_ENTID JSON to run live")
      return
    end
...
function logo_generation_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ERROR_HANDLING_TEST_LOGO_GENERATION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["ERROR_HANDLING_TEST_LOGO_GENERATION_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["ERROR_HANDLING_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["ERROR_HANDLING_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
    idmap = idmap_resolved,
    env = env,
    explain = env["ERROR_HANDLING_TEST_EXPLAIN"] == "TRUE",
    live = live,
