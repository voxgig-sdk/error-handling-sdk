# ErrorHandling SDK test runner

from __future__ import annotations
import os
import json

from errorhandling_sdk.utility.voxgig_struct import voxgig_struct as vs
...
class ErrorHandlingTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ErrorHandlingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ErrorHandlingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ErrorHandlingTestRunner.getenv("ERROR_HANDLING_TEST_LIVE")
        override = ErrorHandlingTestRunner.getenv("ERROR_HANDLING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ErrorHandlingTestRunner.getenv(key)
        ...
        explain = ErrorHandlingTestRunner.getenv("ERROR_HANDLING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ErrorHandlingTestRunner._test_control is not None:
            return ErrorHandlingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ErrorHandlingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ErrorHandlingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ErrorHandlingTestRunner.load_test_control()
...
def load_env_local():
    ErrorHandlingTestRunner.load_env_local()


def env_override(m):
    return ErrorHandlingTestRunner.env_override(m)


def entity_data(v):
    return ErrorHandlingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ErrorHandlingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ErrorHandlingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ErrorHandlingTestRunner.load_test_control()


def live_client_options():
    return ErrorHandlingTestRunner.live_client_options()


def live_delay_ms():
    return ErrorHandlingTestRunner.live_delay_ms()
