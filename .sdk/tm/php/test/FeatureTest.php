...
// ErrorHandling SDK feature test
//
...
class FtCtrl extends ErrorHandlingControl
{
...
class FtClient
{
    ...
    private ErrorHandlingUtility $utility;

    public function __construct(ErrorHandlingUtility $utility, array $options)
    {
    ...
    public function get_utility(): ErrorHandlingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ErrorHandlingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ErrorHandlingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ErrorHandlingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ErrorHandlingConfig::make_feature($name);
    ...
    private static function buildUrl(ErrorHandlingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ErrorHandlingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ErrorHandlingResponse($fetched);
        }
    ...
    private function fail(ErrorHandlingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ErrorHandlingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
