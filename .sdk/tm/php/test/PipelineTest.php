...
// ErrorHandling SDK pipeline test
//
...
{
    private static function utility(): ErrorHandlingUtility
    {
    ...
    private static function ctx(array $over = []): ErrorHandlingContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ErrorHandlingContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ErrorHandlingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ErrorHandlingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ErrorHandlingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ErrorHandlingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ErrorHandlingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ErrorHandlingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ErrorHandlingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ErrorHandlingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ErrorHandlingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ErrorHandlingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ErrorHandlingResponse([]),
            'result' => new ErrorHandlingResult([]),
        ]);
        [, $err] = ErrorHandlingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ErrorHandlingSpec([]),
            'response' => null,
            'result' => new ErrorHandlingResult([]),
        ]);
        [, $err] = ErrorHandlingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ErrorHandlingSpec(['step' => 's']),
            'response' => new ErrorHandlingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ErrorHandlingResult([]),
        ]);
        [, $err] = ErrorHandlingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ErrorHandlingSpec(['step' => 's']),
            'response' => new ErrorHandlingResponse(self::resp(200, ['v' => 1])),
            'result' => new ErrorHandlingResult([]),
        ]);
        [, $err] = ErrorHandlingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ErrorHandlingSpec(['step' => 's']),
            'response' => new ErrorHandlingResponse(self::resp(200, ['v' => 2])),
            'result' => new ErrorHandlingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ErrorHandlingMakeResponse::call($ctx);
    ...
    {
        $preset = new ErrorHandlingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ErrorHandlingSpec([]),
            'response' => new ErrorHandlingResponse([]),
            'result' => new ErrorHandlingResult([]),
        ...
        [$got, $err] = ErrorHandlingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ErrorHandlingResult([])]);
        [, $err] = ErrorHandlingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ErrorHandlingSpec([]), 'result' => null]);
        [, $err] = ErrorHandlingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ErrorHandlingSpec(['step' => 's']),
            'result' => new ErrorHandlingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ErrorHandlingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ErrorHandlingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ErrorHandlingSpec(['step' => 's']),
            'result' => new ErrorHandlingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ErrorHandlingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ErrorHandlingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ErrorHandlingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ErrorHandlingSpec([]),
            'result' => new ErrorHandlingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ErrorHandlingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ErrorHandlingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ErrorHandlingError('boom', 'boom');
        $utility->fetcher = function (ErrorHandlingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ErrorHandlingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ErrorHandlingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ErrorHandlingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ErrorHandlingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ErrorHandlingResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ErrorHandlingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ErrorHandlingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ErrorHandlingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ErrorHandlingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ErrorHandlingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ErrorHandlingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ErrorHandlingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ErrorHandlingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ErrorHandlingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ErrorHandlingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ErrorHandlingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ErrorHandlingResult(['ok' => false])]);
        $this->expectException(ErrorHandlingError::class);
        ErrorHandlingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ErrorHandlingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ErrorHandlingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ErrorHandlingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ErrorHandlingBaseFeature();
        $b = new ErrorHandlingBaseFeature();
        ErrorHandlingFeatureAdd::call($ctx, $a);
        ErrorHandlingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ErrorHandlingBaseFeature
    {
        $f = new ErrorHandlingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ErrorHandlingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ErrorHandlingContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ErrorHandlingSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ErrorHandlingPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ErrorHandlingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ErrorHandlingResponse(['headers' => null]),
            'result' => new ErrorHandlingResult([]),
        ]);
        ErrorHandlingResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ErrorHandlingResultBody::call($ctx);
